...
import 'package:water_jug_challenge/controllers/jug_solver.dart';
import 'package:water_jug_challenge/models/jug_state.dart';

void main() {
  group('JugSolver Tests', () {
    test('Should return "No Solution" when no solution is possible', () {
      final solver = JugSolver(xCapacity: 5, yCapacity: 3);

      List<JugState> result = solver.solveJugProblem(9);
    ...
    test(
      ...
      List<JugState> result = solver.solveJugProblem(1);
    ...
    test(
        ...
        () {
      final solver = JugSolver(xCapacity: 4, yCapacity: 3);

      List<JugState> result = solver.solveJugProblem(2);
    ...
    test(
        ...
        () {
      final solver = JugSolver(xCapacity: 5, yCapacity: 4);

      List<JugState> result = solver.solveJugProblem(10);
    ...
    test('Verifies solution with equal jar capacities', () {
      final solver = JugSolver(xCapacity: 3, yCapacity: 3);

      List<JugState> result = solver.solveJugProblem(3);
    ...
    test('Verifies solution with z being the total capacity', () {
      ...
      List<JugState> result = solver.solveJugProblem(8);

      expect(result.last.x + result.last.y, 8);
    });

    test('Verifies minimum solution for a known case', () {
      final solver = JugSolver(xCapacity: 7, yCapacity: 5);
    ...
    test('Verifies behavior when z is 0', () {
      ...
      List<JugState> result = solver.solveJugProblem(0);
